...
print(names_2013[names_2013$prop == max(names_2013$prop), "name"])
...
number_in_million <- function(name, year) {
  names_year <- names[names$year == year, ]
  percentage <- names_year[names_year$name == name, "prop"]
...
print(
  "0 because I have a male name, but the database contains only female names.")
